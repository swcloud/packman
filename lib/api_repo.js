      ...
      this.languages.forEach(function(l) {
        if (packager[l]) {
          var buildAPackage = function buildAPackage(next) {
            ...
            // For NodeJS and Ruby, we build the api name based on the
            // last dir name from the full path of gaxDir (i.e. final_repo_dir)
            if ((l === 'nodejs' || l === 'ruby') &&
                cleanName.startsWith('google-')) {
              opts.packageInfo.api.name = (l === 'nodejs') ?
                  '@google-cloud/' + lastDirNameInGaxDir : lastDirNameInGaxDir;
            } else {
